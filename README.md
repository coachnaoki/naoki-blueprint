...
| ショート動画（30〜60秒） | `.template-shorts/` | 1080×1920 | 14ステップ |
...
```
public/
├── main/           ← 本編動画（MP4）をここに入れる ★必須
├── inserts/        ← 物理挿入動画（ナレーションを止めて挟み込むクリップ・任意）
├── overlays/       ← オーバーレイ動画（ナレーション中に上に被せるクリップ・任意）
├── opening/        ← OP動画（冒頭に連結するタイトル映像・任意）
├── highlight/      ← 冒頭ハイライト動画（本編切り抜きのベスト部分・任意）
├── images/         ← 挿入画像（任意）
├── bgm/            ← BGM（MP3）をここに入れる ★必須
├── se/             ← SE（効果音）を入れる ★必須
│   ├── 強調/       ← 通常の強調・箇条書き・表など
│   ├── ポジティブ/  ← 前向き・成功・インパクト系
│   └── ネガティブ/  ← 警告・失敗・ツッコミ系
...
| 素材 | 必須？ | 説明 |
|------|--------|------|
| 本編動画 | **必須** | 撮影したMP4ファイルを `main/` に入れる |
| 物理挿入動画 | 任意 | ナレーションを止めて挟み込むクリップを `inserts/` に入れる |
| オーバーレイ動画 | 任意 | ナレーション中に上に被せる補足映像を `overlays/` に入れる |
| OP動画 | 任意 | 冒頭のタイトル・ロゴ映像を `opening/` に入れる |
| ハイライト動画 | 任意 | 完成後の切り抜きベスト映像を `highlight/` に入れる |
