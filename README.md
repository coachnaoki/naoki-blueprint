...
> **📋 更新履歴**: 最新のバージョンアップ内容は [CHANGELOG.md](./CHANGELOG.md) を参照してください。
...
### Q: 2本目の動画を作りたい
`新規作成.sh` を使うと**本体の最新版に自動同期してから**プロジェクトが作られます（ライセンス再認証は不要）：
...
> 新規作成時に `git fetch + git reset --hard origin/main` を自動実行するため、`アップデート.sh` を手動で呼ぶ必要はありません。`projects/` と `.license` は `.gitignore` で守られるので既存プロジェクトは影響を受けません。

**Windows (PowerShell)** または手動でやりたい場合:
```powershell
cd naoki-blueprint
...
npm install
```
※ 手動コピーの場合は事前に `./アップデート.sh` を実行して最新版にしてください。
