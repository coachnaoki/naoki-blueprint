...
### Phase 5: 言い直し検出・カット（step03で「未削除」の場合のみ）

step03で言い直しが未削除と回答された場合、台本とtranscriptを照合して言い直し部分を検出する。

#### 手順
1. 台本にない繰り返し・言い直し部分をリストアップする
   - 同じ内容を2回言っている箇所
   - 途中で言い直している箇所（「えっと、じゃなくて」等）
2. ユーザーに一覧を提示して確認する（**アドリブは残す可能性があるため、必ず確認**）
3. ユーザーがカットOKした部分をffmpegの`trim`+`concat`で物理カット

```bash
# 例: 10.5s〜12.3s をカット
ffmpeg -y -i public/video/input_cut.mp4 -filter_complex \
"[0:v]trim=0:10.5,setpts=PTS-STARTPTS[v1]; \
 [0:a]atrim=0:10.5,asetpts=PTS-STARTPTS[a1]; \
 [0:v]trim=12.3,setpts=PTS-STARTPTS[v2]; \
 [0:a]atrim=12.3,asetpts=PTS-STARTPTS[a2]; \
 [v1][a1][v2][a2]concat=n=2:v=1:a=1[outv][outa]" \
-map "[outv]" -map "[outa]" -c:v libx264 -crf 18 -preset fast -c:a aac -b:a 192k \
public/video/input_cut2.mp4
```

4. **カット後、step04（文字起こし）からやり直す**
5. やり直し後、Phase 1〜4 を再実行する

⚠️ **`-c copy`（ストリームコピー）は使わない。** キーフレーム不整合でフリーズする。必ず再エンコードする。
...
次のステップ → /step06-template（テンプレート設定）
進めますか？
```
