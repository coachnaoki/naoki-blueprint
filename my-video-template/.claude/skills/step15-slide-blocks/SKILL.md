...
### 3. extraCss（ワイプ・テロップ回避CSS）

スライド内の要素がワイプ（右上325x325）やテロップ（下部）と被る場合、`extraCss` でスライド全体のレイアウトを調整できる。

```javascript
// 例: steps×5 でワイプ・テロップ回避
6: {
  extraCss: `
    /* ブロック幅を縮小（ワイプ・テロップ回避） */
...
**ワイプ・テロップ回避の座標計算:**
- スライド座標は1280x720 → 表示1920x1080（1.5倍）
- ワイプ: 右上 top:30 right:30 325x325px（1920x1080空間）→ スライド座標では右217px・上20px・217x217px
- テロップ: 下部 bottom:40（1080空間）→ スライド座標では下部約27px + フォントサイズ分
...
```
✅ Step 15 完了: ブロック分割スライドキャプチャが完了しました。

次のステップ → /step16-slide-timeline（スライドタイムライン）
進めますか？
