...
**すべてのステップを実行する前に、必ずライセンス認証を確認すること。**

1. プロジェクトルートに `.license` ファイルが存在するか確認する
2. **存在しない場合**: ユーザーにライセンスIDの入力を求め、`node scripts/validateLicense.mjs <ID>` を実行する
3. **認証失敗した場合**: ステップの実行を中止し、「ライセンスIDが無効です。発行元に確認してください」と伝える
4. **存在する場合**: `.license` の内容を読み取り、「✅ {name} さん、認証済みです」と表示して続行する

> ⚠️ `.license` ファイルがない状態で動画制作のステップを実行してはならない
