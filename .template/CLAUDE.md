...
| **emphasis3** | — (SVG画像) | width指定 | — (画像) | drop-shadow + バウンス | SVG画像差込 | se/ポジティブ/ |
...
| bounce（下からスライド + scale 0.8→1.1→1.0） | emphasis3 | 20フレーム |
...
// アニメーション: 下からスライド(0→15f) + フェードイン(0→15f) + バウンス scale(0.8→1.1→1.0, 0→10→20f)
// 最大幅: 1400px（画面端の余白100px×2 + 内部余白考慮）。svgFileのwidthプロパティで指定、未指定時1000px
