...
2. **ただし実行直後に必ず以下の案内を返すこと**:
   > ✅ プロジェクト作成完了。
   >
   > ⚠️ **ここで重要**: この Claude Code セッションはテンプレルートで起動されており、新しく作ったプロジェクト（`projects/XXX/`）を認識していません。このまま `/step01-context` 等のステップを叩くと、作成物ではなく**テンプレ本体（.template/）を書き換えてしまう事故**になります。
   >
   > 必ず以下の手順で続けてください:
   >
   > 1. 新しいターミナルウィンドウを開く
...
### スライド生成システム（v2.0 動的生成）
- **生成**: step12-slides-gen が `aislides/slides.html` を動的生成（17テンプレ固定方式は廃止）。ターゲット・トンマナに応じてカラー・フォント・レイアウトを Claude が判断
- **キャプチャ**: `scripts/captureSlides.mjs` で Puppeteer 実行 → `public/slides/` に出力（1920×1080 × deviceScaleFactor 2 = 4K）
- **ブロック分割**: HTML の `data-blocks="N"` / `data-block-index="K"` 属性で自動検出。カード/ステップ系は段階表示画像を生成
- **視認性**: YouTube + スマホ縦画面前提。最低文字サイズ 40px、本文最大2行、word-break: keep-all で単語の途中で改行禁止
- **テロップ域**: 下 200px は確保（content height ≤ 880px）
...
- **コンポジション拡張（step09-composition）**: 改変したアニメーション・コンポーネントを反映する
...
style ファイルと CLAUDE.md のルールが矛盾する場合、**受講生の改変が優先**される (自己責任)。ただし固定値ルール (色パレット基本・14文字制限等) を覆す改変は警告すること。
...
### 動画完成時の確認 (step19-opening / step20-highlight-final)
