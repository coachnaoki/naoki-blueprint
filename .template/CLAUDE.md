...
### モデル切り替え（画像確認ステップ）
以下のステップでは画像認識が頻繁に発生する。**ステップ開始時にユーザーへsonnetへの切り替えを案内すること。**

```
⚠️ このステップでは画像の確認が多いため、sonnetへの切り替えを推奨します。
　 `/model sonnet` と入力してください。
```

対象ステップ：
- **step08-template**: テロップのフォント・色のプレビュー確認
- **step15-wipe**: ワイプ位置調整（スクショで顔の位置確認）
- **step16-images**: 画像の配置・内容確認

理由: Opusは画像認識の精度がsonnetとほぼ同等だがコスト3〜5倍。画像確認が多いステップではsonnetが最適。
