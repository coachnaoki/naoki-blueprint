...
step19-opening         → 冒頭にOP連結＋本編レンダリング（1回目・任意のOP）
step20-highlight-final → 本編MP4からハイライト自動抽出＋最終レンダリング（任意）
```

### スライド生成システム
- **テンプレート**: `aislides/slides.html` に17種類のHTMLテンプレート（title / three-cards / three-tactics / two-columns / steps / big-message / closing / quote / before-after / stats / checklist / timeline / ranking / versus / highlight-box / agenda / section）
...
2. **動画構築**（step07〜09）: テンプレート設定 → テロップ → コンポジション拡張（Remotion Studio は step05 で起動済み、ホットリロードで反映）
...
5. **BGM・冒頭連結・出力**（step18〜20）: BGM挿入 → OP連結+本編レンダリング → ハイライト自動抽出+最終レンダリング
...
| **emphasis3** | — (SVG画像) | width指定 | — (画像) | drop-shadow + 下からスライド | SVG画像差込 | se/ポジティブ/ |
...
| アニメーション | テンプレート | 時間 |
|---|---|---|
| slideUp（下から上・フェードイン） | emphasis / emphasis2 / emphasis3 / section / negative2 | 10フレーム |
...
- **EmphasisTelop3**: SVG画像テロップ（ロゴ/意匠化したキラーワードを差込）
...
// emphasis3: SVG画像テロップ（キラーワードをロゴ/意匠化したSVG画像を差し込む）
// svgFileプロパティでファイル名を指定、public/telop-svg/ 配下から読み込む
// <Img src={staticFile(\`telop-svg/${svgFile}\`)} style={{ width, height: "auto" }} />
// アニメーション: 下からスライド + フェードイン（10フレーム、他の強調系と統一）
// 最大幅: 1400px（画面端の余白100px×2 + 内部余白考慮）。svgWidthプロパティで指定、未指定時1000px
// ドロップシャドウ: filter: "drop-shadow(0px 4px 12px rgba(0,0,0,0.3))"
// 背景: 透過（SVG側で背景色を塗らない。動画に重ねる想定）
// 文字の縁取り: 太い黒縁 (stroke="#000000" strokeWidth=22前後) を SVG側で指定推奨
// フォントは不要（画像）、色は画像自体のカラーに依存
```
...
| **emphasis3** | `filter: "drop-shadow(0px 4px 12px rgba(0,0,0,0.3))"` (SVG画像の外側 `<div>` に指定) |
...
| line_cta | 66 |
| heading | 54 |
| **emphasis3** | **— (SVG画像、width指定)** |

### 1行の文字数制限（必須・絶対遵守）

横動画は1行のみ（`\n` での2行化は未対応）。**1チャンクの全角文字数が下表の上限を1字でも超えたら必ず分割する。**

| fontSize | テンプレート | 最大文字数 |
|----------|-----------|-----------|
| 84 | normal / normal_emphasis / third_party | **20文字** |
| 96 | negative | **17文字** |
| 108 | theme | **16文字** |
| 122 | emphasis / emphasis2 / section / negative2 | **14文字** |
| — | emphasis3 (SVG画像) | **文字数制限なし / 最大幅1400px** |

- **生成時に必ず字数を数えてから text に入れる。** 「だいたいこのくらい」での目分量禁止
- **third_party は表示時に `｢ ｣` が自動付与されるので、text 自体は18字以内に収める**
- **分割の優先順位**:
  1. 助詞（は/の/に/を/が/で/も/て/と）の後で分割
  2. それでも入らなければユーザーに「この文は長すぎるのでどう分割しますか？」と確認
- ❌ **NG**: 句読点（、。）で分割しようとする → テロップから句読点は既に削除されているので存在しない
- ❌ **NG**: 制限超過のまま出力する → 1字オーバーでも分割必須
...
#### emphasis vs emphasis2 vs emphasis3 の使い分け（必須）
| 条件 | テンプレート | 例 |
|------|------------|-----|
| **数字+単位**を含む | **emphasis2** | 「1000万円」「3倍」「500人」「10年」 |
| **感情・概念の強調**（数字なし） | **emphasis** | 「人生が変わります」「最強ツール」「おすすめ」 |
| **ロゴ/意匠化したキラーワードをSVG画像で差し込む** | **emphasis3** | 「Japanese Ninja」「カナダ式」「主観の罠」「逆転」等のロゴSVG |

- テキストに具体的な数字+単位（円・万・人・年・倍・%・回・件など）が含まれていたら **emphasis2**
- それ以外のポジティブ強調（文字表現）は **emphasis**
- Naokiが事前に SVG 意匠を用意しており、キラーワードをロゴとして画面に置きたい場合のみ **emphasis3**（`public/telop-svg/` 配下のファイルを指定）
...
| ロゴ/意匠化したSVG画像での差込 | SVG画像テロップ | EmphasisTelop3 | ポジティブ/ |
