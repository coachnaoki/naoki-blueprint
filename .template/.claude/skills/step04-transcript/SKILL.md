...
> **重要**: このステップではカット前の元動画に対して文字起こしを行う。これは次のstep05（台本照合）とstep06（一括カット）で、言い直しや無音区間を正確に特定するため。カット後の最終的な文字起こしはstep07で行う。

## 前提条件
- Step 02（素材チェック）が完了していること、またはStep 03（動画差し込み）で結合済み動画があること
- 対象動画が `public/video/` に存在すること（元動画 or 差し込み結合後の動画）
...
以下の優先順で対象動画を決定する：
1. Step 03で差し込み結合した動画（`*_with_clips.mp4`）があればそれを使う
2. なければ元動画（`public/video/` 内の `.mp4`）を使う
...
result = mlx_whisper.transcribe('public/video/対象動画.mp4', word_timestamps=True, path_or_hf_repo='mlx-community/whisper-small-mlx')
