...
`public/video/` 内の全 `.mp4` ファイルを一覧表示し、`ffprobe` で各ファイルの解像度・FPS・長さを確認する。

**複数の動画ファイルがある場合**、ユーザーにそれぞれの役割を確認する：
- メイン動画（撮影素材）
- 差し込み動画（step03で結合する素材）
- クリップ動画（step12でオーバーレイする素材）

確認後、`video-context.md` の制作設定に動画ファイルの役割を記録する：
```markdown
## 動画ファイル
- メイン動画: main.mp4
- 差し込み動画: intro.mp4（あれば）
- クリップ動画: demo1.mp4, demo2.mp4（あれば）
```
