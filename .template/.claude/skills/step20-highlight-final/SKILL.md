...
```
[Series]
  ├─ 1. ハイライト（step19 MP4から切り抜いた public/highlight/*.mp4）
  ├─ 2. OP（public/opening/*.mp4）※あれば
...
ffmpeg -ss 50.000 -i public/output/step19_main.mp4 \
  ...
  -pix_fmt yuv420p \
  public/highlight/highlight_main.mp4
```
...
ffmpeg -f concat -safe 0 -i /tmp/concat.txt -c copy public/highlight/highlight_main.mp4
```

### 3. ハイライト長さを測定

```bash
ffprobe -v quiet -show_entries format=duration -of default=noprint_wrappers=1 public/highlight/highlight_main.mp4
```
