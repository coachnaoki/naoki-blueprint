...
- キーは macOS Keychain に登録済み（初回のみ: `security add-generic-password -a $USER -s gemini_api_key -w`）
- 実行前に `export-gemini` を叩いて `GEMINI_API_KEY` を環境変数にロード済み
...
→ OK: 次のステップ → /step17-special-components（特殊コンポーネント）
```
