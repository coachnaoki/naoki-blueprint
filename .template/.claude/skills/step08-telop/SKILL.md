...
| ロゴ/意匠化したSVG画像で差し込むキラーワード（Naokiが事前準備） | emphasis3（svgFile指定） |
...
export interface TelopEntry {
  ...
  svgFile?: string;                      // emphasis3 専用。public/telop-svg/ 配下のファイル名（例: "逆転.svg"）
  svgWidth?: number;                     // emphasis3 専用。表示幅(px)。未指定時は templateConfig.emphasis3.defaultWidth (1000)。最大 1400
}

export const telopData: TelopEntry[] = [
  // 通常テロップ
  { text: "独学で上達するのは難しい", startFrame: 120, endFrame: 210, template: "normal" },
  // SVG画像テロップ (emphasis3)
  { text: "逆転", svgFile: "逆転.svg", svgWidth: 1000, startFrame: 300, endFrame: 390, template: "emphasis3" },
];
```

### emphasis3（SVG画像テロップ）の使い方

- `template: "emphasis3"` を指定し、`svgFile` に `public/telop-svg/` 配下のファイル名を渡す
- `svgWidth` は省略可（デフォルト1000px）。最大1400pxまで
- `text` は空文字でもよいが、読みやすさのため **SVGに描かれている文字列を入れておく**（コード上で何が出るか分かる）
- SVGファイルが存在しない場合は `public/telop-svg/` に配置してもらうようユーザーに依頼する
- emphasisWord は emphasis3 では無視される
