...
1. `ls -la public/images/inserts/ public/images/overlays/` で画像素材を確認
...
3. **キーを受け取ったら、`.env` ファイルを自動作成**
   ...
   ```
   作成後、ユーザーに「✅ `.env` を作成しました。これで画像生成が使えます。`references/policy-compliance.md` のNGリストに該当するプロンプトは生成しません」と報告する。
...
特に以下に注意:
- **実在の人物名・有名人名は絶対に書かない**（例: フェデラー、マスク、有名選手等 → `fictional adult athlete` に置き換え）
...
業種別の安全な代替例とフレーズ集は `references/policy-compliance.md` を参照。
...
`public/images/inserts/` に配置した画像を使う。位置は以下の固定値（変更不可）：
