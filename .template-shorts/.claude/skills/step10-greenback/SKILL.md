...
# Step 10: グリーンバック背景置換（任意・ショート動画用）

ライブ動画がグリーンバック撮影の場合、背景を画像に置換する。**グリーンバックの品質に応じて2つの方式を選択**する。

> ショート動画は 1080×1920 縦動画。背景画像も縦長で用意すること。

## 前提条件
- Step 09（コンポジション構築・登録）が完了していること
- 元動画がグリーンバックで撮影されていること
- 背景画像（縦長 1080×1920 推奨）が `public/images/overlays/` に配置されていること
...
| 判定 | 採用手法 | 処理時間（1080×1920・10秒動画） |
|------|---------|-------------------|
| **均一（RGB差 < 20）** | A: ffmpeg chromakey | 10秒 |
| **ムラあり（RGB差 ≥ 20）** | **B: rembg u2net パイプライン** | 約4.5分 |
...
上のサンプリングで取得した緑色（例: `#65DC08`）を `chromakey` のHEXに指定する。デフォルトの `0x00FF00`（純緑）は実際の撮影緑と合わず人物まで削れるので**必ず実色を使う**。
...
#### Step A-2: MainComposition.tsx に背景+透過動画を配置（1080×1920）
...
**進捗表示例:**
```
入力: 1080x1920@59.94 604frames  model=u2net  edge_px=3
