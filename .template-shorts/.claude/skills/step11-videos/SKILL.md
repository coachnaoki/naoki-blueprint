...
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(npx tsc *), Bash(npx remotion still *), Bash(rm *), Bash(ls *), Bash(ffprobe *), Bash(node *)
---
...
- **z-index**: 5以下（テロップの下）
- **全画面動画の表示中は見出しバナーを非表示にする**

### HeadingBannerの非表示実装

全画面動画（物理挿入・オーバーレイ問わず）が表示されている間、HeadingBannerを非表示にする。
...
- テロップは **本編のフレーム軸のまま** 動くため、タイムスタンプは崩れない
...
- これで transcript_words / telopData の既存ロジックは一切変更不要
...
他にも動画クリップを挿入しますか？
→ はい: 同じステップを繰り返す
→ いいえ: 次のステップ → /step12-images /step12-images
```
