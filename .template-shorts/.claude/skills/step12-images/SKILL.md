...
2. 感情が動くテロップを5〜10個マークする（ショート動画は短いので少なめ）
3. 以下を除外する：
...
4. 動画クリップ・BulletList表示中は画像を入れない
5. 動画全体にバランスよく分散させる
```
...
全画面画像が表示されている間は、見出しバナーが画像の上に重なって邪魔になるため非表示にする。

**実装方法**: HeadingBanner に全画面画像のフレーム範囲を渡して条件分岐する。
...
→ OK: 次のステップ → /step13-special-components（特殊コンポーネント）
```
